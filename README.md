...
- The answer from libraries that do this is almost always that it's same interface as HTML, i.e. `<md-button>` defines `type="reset"` too and the implementation is often just a pass-through. Laaaaaame.
  - This is extremely wasteful for developers having to relearn and build trust with these mostly-the-same interfaces, e.g. unlike HTML where `submit` is button's default type, the Shoelace design system surprisingly sets `button` as the default.
